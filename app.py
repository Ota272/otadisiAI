...
def _plot_shap_waterfall(app: dict):
    """
    Plotly-waterfall по base value + SHAP (топ факторов).
    Работает только если backend возвращает shap_base_value и all_shap_values.
    """
    base = app.get("shap_base_value")
    all_shap = app.get("all_shap_values", {})
    if base is None or not all_shap:
        st.caption("Waterfall недоступен: нет `shap_base_value` или `all_shap_values`.")
        return

    # Берём топ-8 по |SHAP|
    items = sorted(all_shap.items(), key=lambda x: abs(x[1]), reverse=True)[:8]
    ...
    nice = [FEAT_LABELS.get(k, k) for k in labels]
    measures = ["absolute"] + ["relative"] * len(vals) + ["total"]
    x = [base] + vals + [0]
    text = [f"{base:+.2f}"] + [f"{v:+.2f}" for v in vals] + [""]
    y = ["Base (ожидаемое)"] + nice + ["Итог (base + SHAP)"]
    ...
    fig = go.Figure(
        go.Waterfall(
            ...
            decreasing={"marker": {"color": "#b5001f"}},
            totals={"marker": {"color": "#003580"}},
        )
    )
    fig.update_layout(
        title="SHAP Waterfall (base value + вклады факторов)",
        height=360,
        plot_bgcolor="#f8faff",
        paper_bgcolor="#ffffff",
        margin=dict(t=50, b=30, l=10, r=60),
        font=dict(family="Golos Text", size=11),
        xaxis_title="Вклад в предсказание (в шкале модели)",
    )
...
with st.sidebar:
    ...
    st.divider()
    with st.expander("🧪 Диагностика окружения (LLM)", expanded=False):
        _k = os.getenv("GEMINI_API_KEY", "")
        st.write("GEMINI_API_KEY обнаружен:", bool(_k))
        if _k:
            st.caption(f"Длина ключа: {len(_k)} (значение не показывается)")
        st.caption(f".env загружен из: {_ENV_LOADED_FROM or 'не найден/не прочитан'}")
        st.caption("Если False — остановите и запустите Streamlit заново после правок в `.env`.")
...
with tab1:
    ...
    with st.expander("✏️ Подать заявку на скоринг", expanded=True):
        ...
        st.caption(
            ...
            "Gemini LLM проанализирует каждый документ на соответствие правилам субсидирования МСХ РК "
            ...
            if not man_bin.strip() or not man_company.strip():
                ...
                if uploaded_docs:
                    ...
                    if total_mb_chk > 200:
                        ...
                        with st.spinner(
                            f"🧠 Gemini анализирует {len(uploaded_docs)} документ(ов) "
...
with tab3:
    ...
    if not apps:
        ...
        if app is None:
            ...
            with col_btn:
                if st.button("🤖 Получить заключение Gemini", use_container_width=True, key=f"gemini_btn_{selected_id}"):
                    if gemini_key:
                        ...
                        st.warning("Установите переменную GEMINI_API_KEY для работы Gemini.")
                ...
                <div style="background:#f9faff; border:1.5px solid {verdict_color};
                    ...
                    <div style="display:flex; align-items:center; gap:10px; margin-bottom:12px;">
                        ...
                        <span style="font-family:'Montserrat',sans-serif; font-weight:700;
                            ...
                            Экспертное заключение — Gemini AI
                        </span>
            ...
            with prof_col1:
                ...
                st.caption("Примечание: динамика ниже является демонстрационной симуляцией (в прототипе нет подключённой отчётности предприятия).")
                ...
                # Radar chart
                shap_vals = app.get("all_shap_values", {})
                feature_labels = {
                    "gross_output_growth_yoy": "Рост продукции",
                    "pedigree_ratio": "Племенное поголовье",
                    "land_to_livestock_ratio": "Земля",
                    "historical_survival_rate": "Выживаемость",
                    "debt_load_ratio": "Долг (инверт.)",
                    "subsidy_dependence_index": "Независимость",
                    "years_in_operation": "Стаж",
                    "veterinary_compliance": "Ветеринария",
                }

                radar_labels = list(feature_labels.values())
                raw_features = app.get("raw_features_used", {})
                radar_vals_raw = {
                    "gross_output_growth_yoy": (raw_features.get("gross_output_growth_yoy", 0) + 0.3) / 1.3 * 100,
                    "pedigree_ratio": raw_features.get("pedigree_ratio", 0.5) * 100,
                    "land_to_livestock_ratio": min(raw_features.get("land_to_livestock_ratio", 2) / 8 * 100, 100),
                    "historical_survival_rate": raw_features.get("historical_survival_rate", 0.85) * 100,
                    "debt_load_ratio": max(0, (1 - raw_features.get("debt_load_ratio", 1.5) / 5)) * 100,
                    "subsidy_dependence_index": (1 - raw_features.get("subsidy_dependence_index", 0.3)) * 100,
                    "years_in_operation": min(raw_features.get("years_in_operation", 5) / 20 * 100, 100),
                    "veterinary_compliance": raw_features.get("veterinary_compliance", 0.85) * 100,
                }
            ...
            with prof_col2:
                ...
                <div style="background:#f0f4ff; border:1px solid #dde3ef; border-radius:8px;
                    ...
                    <b>ℹ️ О SHAP:</b> Это не «данные из PDF». Числа ± — вклад признака в балл модели <b>XGBoost</b>
                    (SHAP TreeExplainer). Вход модели: числа из формы заявки, опциональных полей и дефолтов;
                    при успешном разборе PDF часть признаков могла уточниться из текста документов.
                    Рекомендация ИИ — ориентир; финальное решение принимает комиссия.
                </div>
                """, unsafe_allow_html=True)
                _dc = int(app.get("documents_text_chars") or 0)
                _de = app.get("documents_extracted_ok")
                if _dc > 0:
                    ...
                    st.caption("Документы: к этой записи файлы не прикреплялись или заявка из синхронизации ГИСС без вложений.")

                # Читаем top_positive_factors и top_negative_factors из нового формата API
                ...
                for item in all_factors:
                    ...
                    <div class="shap-item {cls}">
                        <span class="shap-val">{sign}{shap_val:.1f}</span>
                        <div>
                ...
                # SHAP waterfall (Plotly) — из all_shap_values
                all_shap = app.get("all_shap_values", {})
                if all_shap:
                    sorted_shap = sorted(all_shap.items(), key=lambda x: abs(x[1]), reverse=True)[:6]
                    FEAT_LABELS = {
                        "gross_output_growth_yoy": "Рост продукции",
                        "pedigree_ratio": "Племенное поголовье",
                        "historical_survival_rate": "Выживаемость стада",
                    ...
                    labels_shap = [FEAT_LABELS.get(k, k)[:22] for k, _ in sorted_shap]
                    vals_shap = [v for _, v in sorted_shap]
                ...
                # Более корректный waterfall: base value + вклады
                st.divider()
                _plot_shap_waterfall(app)
