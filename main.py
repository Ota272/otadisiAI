...
from shap_integration import ScoringEngine, extract_features_from_documents, extract_text_from_pdf, generate_gemini_expert_opinion
...
VALID_API_KEYS = {
    ...
    "sk-msgov-giss-integration-xyz": "ГИСС — Интеграционный сервис",
}
...
def score_application(
    ...
    result = engine.score_farmer(feature_dict)
    ...
    _applications_db.append(response_data)
...
async def _gemini_ocr_pdfs(pdf_items: list[tuple[str, bytes]], api_key: str) -> str:
    """
    Читает PDF файлы напрямую через Gemini Files API.

    Gemini нативно понимает PDF включая сканированные (встроенный OCR).
    Используется как fallback когда pdfplumber вернул пустой текст.

    Логика: загружаем каждый файл в Files API, затем одним запросом
    просим Gemini извлечь весь текст из всех файлов.
    """
    ...
    for fname, pdf_bytes in pdf_items:
        ...
        try:
            ...
            # Gemini читает PDF нативно — включая сканы
    ...
    # Удаляем загруженные файлы из Gemini Files API (бесплатно, но занимают квоту)
...
async def score_with_documents(
    ...
    Слой 3: LLM извлекает фичи из PDF → обновляет вектор перед XGBoost
    ...
    if documents:
        ...
        if missing and gemini_api_key:
            print(f"[gemini_ocr] pdfplumber не смог прочитать {len(missing)} файл(ов) — пробуем Gemini Files API...")
            ocr_text = await _gemini_ocr_pdfs(missing, gemini_api_key)
            if ocr_text.strip():
    ...
    # ── Слой 4: LLM-compliance + адаптивное взвешивание ──────────
    #
...
def sync_from_giss(
    ...
    """Синхронизация с ГИСС: получаем заявки и сразу скорим их."""
    import random
    ...
    for _ in range(random.randint(3, 7)):
        ...
        result = score_application(f, api_key, engine)
...
def get_analytics_summary(api_key: str = Depends(verify_api_key)):
    """Сводная аналитика для дашборда МСХ."""
    if not _applications_db:
        return {"message": "Нет данных для анализа"}

    scores  = [a.get("score", 0) for a in _applications_db]
    amounts = [a.get("requested_amount", 0) for a in _applications_db]
    zone_counts = {"green": 0, "yellow": 0, "red": 0}
    for a in _applications_db:
        z = a.get("zone", "yellow")
        if z in zone_counts:
            zone_counts[z] += 1

    return {
        "total_applications":    len(_applications_db),
