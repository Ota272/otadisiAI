...
def extract_text_from_pdf(pdf_path: str) -> str:
    ...
    В production использовать: pdfplumber или pymupdf (fitz).
    """
    try:
        import pdfplumber
        with pdfplumber.open(pdf_path) as pdf:
            texts = []
            for page in pdf.pages[:10]:  # Первые 10 страниц максимум
...
3. Если compliance не проводился — формулируй: «пакет документов в системе не проанализирован по тексту», а не «нет данных по предприятию».
...
Напиши экспертное заключение по данной заявке, строго опираясь на приведённые Правила субсидирования МСХ РК."""
